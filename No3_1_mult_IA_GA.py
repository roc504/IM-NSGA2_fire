...
@File    : mult_test.py
@Author  : roc
@Time    : 2023/9/10 16:09
"""
"引用No3_0"
import time
# import numpy as np
from tqdm import tqdm, trange
from No3_0_readstripe import *
import geatpy as ea
from pylab import *
...
#融合所有covered1-17到covered_all
'''time_start_1 = time.time()
covered_all = []
for each in tqdm(range(1,18),desc='it\s merge process'):
    real_path = (r'D:\stripe_txt\covered_all\covered'+str(each)+'.npy')
    real_data = np.load(real_path, allow_pickle=True)  # 类型是numpy array
    covered_all.append(real_data)
np.save(r'D:\stripe_txt\covered_all\covered_all.npy', covered_all)
time_end_1 = time.time()
print("运行时间："+str(time_end_1 - time_start_1)+"秒")
print('npy merge finished')'''
...
def variation(solution,front,distance):
    ...
    for ii in range(solution.shape[0]):  # 遍历所有个体
        ...
        deta = deta0 / (gen_no + 1)  # 最好是大于1的数。刚开始迭代时,deta较大,初始值可以是上下限的中间值，随着迭代次数变多,deta减少
        #变异
...
def refresh():
    ...
    创建一般新生种群并返回
    """
    bf = np.random.randint(0, no4, (NIND, D))  #创建0-1000000很大的随机数，然后约束边界生成随机新种群
...
"""=========================免疫算法参数设置========================"""
deta0=300#5# 最好是大于最大代数的整数。刚开始迭代时,deta较大,初始值可以是上下限的中间值，随着迭代次数变多,deta减少
pm=0.7#变异率
gen_no=0#初始代数
# D=25#维度，几个卫星
max_gen=150#最大代数
...
'''=====================生成结果==================='''
#把function123存在txt中
with open('D:\\stripe_txt\\st\\result\\function1.txt', "w") as file:
    # file.write('front:'+"\n")
    for s in function1_values:
        file.write(str(s) + "\n")

with open('D:\\stripe_txt\\st\\result\\function2.txt', "w") as file:
    # file.write('front:'+"\n")
    for s in function2_values:
        file.write(str(s) + "\n")

with open('D:\\stripe_txt\\st\\result\\function3.txt', "w") as file:
    # file.write('front:'+"\n")
    for s in function3_values:
        file.write(str(s) + "\n")

#把front存在txt中
with open('D:\\stripe_txt\\st\\result\\non_dominated_sorted_solution.txt', "w") as file:
    # file.write('front:'+"\n")
    for s in non_dominated_sorted_solution:
        file.write(str(s) + "\n")
#把最终的phenx存在txt中
np.savetxt("D:\\stripe_txt\\st\\result\\phenx.txt", Phen,fmt='%d',delimiter=',')
...
plt.savefig('D:\\stripe_txt\\st\\result\\covered-cost-'+str(max_gen)+'.png')
...
plt.savefig('D:\\stripe_txt\\st\\result\\covered-time-'+str(max_gen)+'.png')
...
plt.savefig('D:\\stripe_txt\\st\\result\\cost-time-'+str(max_gen)+'.png')
